...
## Operational context and guarded refresh (2026-09-23)

Status: ATTEMPTED for the complete UI/production flow. Controlled gateway and workflow outcome tests pass; authenticated production UI verification remains unavailable in this session.

- Read-only gateway enrichment adds current `production_components` review state to canonical orders and current lifecycle/follow-up fields to the existing session's drafts. IDs come exclusively from the identity/session context RPC, ambiguous identities are excluded, reads are batched/paginated, and errors fail the refresh rather than advertising stale data as fresh. No migration or order/payment mutation is involved.
- A currently confirmed case order can select review artwork, awaiting customer approval, pending-payment or ready-pickup actions. Shipping/after-sales questions use the confirmed order's status. A complaint keeps precedence, and shipped/completed/collected orders never close a conversation automatically.
- Active current-session drafts supply a draft action. Due follow-up requires enabled scheduling, a sent link/follow-up, an elapsed next date, no recorded response, and no newer inbound than the last send. Converted/closed drafts and changed sessions are excluded. A due schedule may surface a manually waiting conversation only when the due date is later than its review; snooze/resolve decisions remain authoritative. No follow-up message is sent implicitly.
- Short acknowledgement-only messages such as `Ok baik terima kasih` show a closure-review action with no recycled product question. Human confirmation is still required to resolve; text containing another request or complaint does not qualify.
- Browser refresh runs every 60 seconds while visible and idle. It refreshes all loaded pages plus the selected detail, aborts application after edits/navigation/actions, preserves pagination, and pauses during unsaved work, focused inputs or confirmation dialogs. On error the old view remains with a visible warning.
- Coverage is still bounded: loaded conversations, up to 8 recent orders per source per customer, and up to 5 drafts from the current session. These are not global totals or proof that every historical order is linked. Provider freshness remains dependent on existing Shopee/courier/inbox ingestion.

Validation: `check-ai-dashboard.mjs`, `check-ai-work-queue.mjs`, `check-ai-workflow.mjs`, `check-ai-workflow-gateway.mjs`, Admin TypeScript and root production build. Gateway smoke uses the actual handler with controlled external reads and verifies response actions, permission rejection and source failure; it does not impersonate a live admin. No real customer message, order, payment or dashboard review was changed. Production function version/source verification and a production branch update alone do not establish end-to-end production behavior.

Deployment record: Order System `admin-ai-dashboard` version 6 is ACTIVE with JWT verification enabled; all five deployed source files were read back and compared to this patch. The frontend/backend source patch targets `production` after parent `319d3f2ad2aeb7c50ac6f928b7a669aca92e52ea`. Authenticated live dashboard smoke remains pending; retain ATTEMPTED for the complete flow.
