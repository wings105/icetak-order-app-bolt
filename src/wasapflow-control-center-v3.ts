...
const TAB_KEY = 'wf_v3_tab';
const DRAFT_KEY = 'wf_v3_draft';
...
const FIELDS = ['customer_name','phone','order_id','order_total','date_need','order_link','payment_link','review_link','tracking_number','courier','tracking_link','pickup_location','otp_code','magic_link','expiry_minutes','support_phone'];
const SAMPLE: Any = {
  customer_name:'Zaim', phone:'60129554732', order_id:'IC260707-3904', order_total:'RM35', date_need:'25/07/2027',
  order_link:'https://icetak.bolt.host/?order=o_xxx', payment_link:'https://icetak.bolt.host/?order=o_xxx&page=payment',
  review_link:'https://icetak.bolt.host/review/demo', tracking_number:'MY123456789', courier:'SPX',
  tracking_link:'https://spx.co/track/MY123456789', pickup_location:'Bandar Baru Pasir Puteh',
  otp_code:'123456', magic_link:'https://icetak.bolt.host/?login=demo', expiry_minutes:'10', support_phone:'60179860656'
};

let cache: Any | null = null;

function token(){ return sessionStorage.getItem('admin_access_token') || sessionStorage.getItem('admin_session') || ''; }
function tab(){ return sessionStorage.getItem(TAB_KEY) || 'connection'; }
function setTab(v:string){ sessionStorage.setItem(TAB_KEY, v); }
...
function setDraft(k:string,v:string){ const d=draft(); d[k]=v; sessionStorage.setItem(DRAFT_KEY, JSON.stringify(d)); }
function getDraft(k:string,fb=''){ return draft()[k] ?? fb; }
...
function toast(msg:string,bad=false){
  ...
  el.textContent = msg;
  Object.assign(el.style,{position:'fixed',zIndex:'999999',left:'50%',bottom:'24px',transform:'translateX(-50%)',background:bad?'#b91c1c':'#166534',color:'#fff',padding:'12px 16px',borderRadius:'12px',fontWeight:'900',boxShadow:'0 16px 48px rgba(0,0,0,.25)'});
  setTimeout(()=>el?.remove(),4800);
}
function copy(v:string){ navigator.clipboard.writeText(v).then(()=>toast('Copied')).catch(()=>toast('Copy failed', true)); }
...
function normalizeRule(r:Any){
  const b = BLUEPRINTS.find(x=>x.event_type===r.event_type) || BLUEPRINTS.find(x=>x.name===r.template_name);
  return {
    ...r,
    label:r.label || b?.label || r.event_type,
    enabled:r.enabled !== false,
    freeform_enabled:r.freeform_enabled !== false,
    template_enabled:r.template_enabled !== false,
    freeform_text:r.freeform_text || (b ? freeformFrom(b) : ''),
    template_name:r.template_name || b?.name || '',
    template_language:r.template_language || 'ms',
    template_params:arr(r.template_params?.value || r.template_params || b?.params),
    available_fields:arr(r.available_fields || b?.params)
  };
}
...
async function load(force=false){
  ...
  cache = {status, rules:ruleList, templates:templates.templates || [], outbox:outbox.outbox || []};
  return cache;
}

function badge(templates:Any[], name:string){
  const t = templates.find(x => String(x.name).toLowerCase() === String(name).toLowerCase());
  if(!t) return '<span class="wf-badge warn">not synced</span>';
  const s = String(t.status || 'SYNCED').toUpperCase();
  return `<span class="wf-badge ${s==='APPROVED'?'ok':'warn'}">${esc(s)}</span>`;
}
function templateOptions(templates:Any[], selected:string){
  const names = Array.from(new Set([...BLUEPRINTS.map(b=>b.name), ...templates.map(t=>t.name)]));
...
async function openPanel(){
  ...
  wrap.innerHTML = `<div class="wf-shell"><aside><b>WhatsApp Control</b><small>Order notification OS</small><button data-tab="connection">⚙️ Connection</button><button data-tab="rules">✉️ Notification Rules</button><button data-tab="templates">📋 Meta Templates</button><button data-tab="test">🧪 Send Test</button><button data-tab="logs">📜 Logs</button></aside><section class="wf-main"><header><div><small>UNIFIED INBOX + WASAPFLOW</small><h2>WhatsApp Notification Control Center</h2></div><button id="wfClose">×</button></header><main><div class="wf-loading">Loading…</div></main></section></div>`;
  wrap.querySelector<HTMLButtonElement>('#wfClose')!.onclick = () => wrap?.remove();
  try { render(wrap, await load()); } catch(e:any){ wrap.querySelector('main')!.innerHTML = `<p class="wf-error">${esc(e.message)}</p>`; }
}

function render(wrap:HTMLElement,data:Any){
  wrap.querySelectorAll<HTMLButtonElement>('[data-tab]').forEach(b=>{ b.classList.toggle('active', b.dataset.tab===tab()); b.onclick=()=>{ setTab(b.dataset.tab!); render(wrap,data); }; });
...
function connectionHtml(d:Any){
  const cfg = d.status.configured || {};
  const connected = !!cfg.partner_key && !!cfg.waba_id;
  return `<section class="wf-metrics"><article><b>${connected?'Connected':'Credential Needed'}</b><span>WasapFlow</span></article><article><b>${d.templates.length}</b><span>Synced templates</span></article><article><b>${d.rules.length}</b><span>Rules</span></article><article><b>${d.outbox.length}</b><span>Logs</span></article></section><section class="wf-card"><h3>Connection Settings</h3><p>Webhook inbound utama kekal di Unified Inbox. Project order ini check 24H dan send notification.</p><form id="wfSettings" class="wf-form"><label>Enable notification<select name="enabled"><option value="true">Enabled</option><option value="false">Disabled</option></select></label><label>WF Base URL<input name="base_url" value="${esc(getDraft('base_url', d.status.base_url || 'https://officialapi.wasapflow.com/bridge/v1'))}"></label><label>Partner Key <small>${cfg.partner_key?'configured ✅':'missing ⚠️'}</small><input name="partner_key" type="password" value="${esc(getDraft('partner_key'))}" placeholder="wf_xxx"></label><label>WABA ID <small>${cfg.waba_id?'configured ✅':'missing ⚠️'}</small><input name="waba_id" value="${esc(getDraft('waba_id'))}" placeholder="123456789"></label><label>Webhook Secret <small>${cfg.webhook_secret?'configured ✅':'missing ⚠️'}</small><input name="webhook_secret" type="password" value="${esc(getDraft('webhook_secret'))}" placeholder="WasapFlow webhook secret"></label><label>Language<input name="default_language" value="${esc(getDraft('default_language','ms'))}"></label><label>Unified Inbox 24H URL<input name="unified_inbox_24h_url" value="${esc(getDraft('unified_inbox_24h_url',UNIFIED_24H_URL))}"></label><label>Unified Inbox 24H Key <small>secret</small><input name="unified_inbox_24h_key" type="password" value="${esc(getDraft('unified_inbox_24h_key'))}" placeholder="Bearer key"></label><label>Customer App URL<input name="customer_app_base_url" value="${esc(getDraft('customer_app_base_url',location.origin))}"></label><label>Order Project Webhook URL <small>optional log only</small><input readonly value="${esc(supabaseUrl + '/functions/v1/wasapflow-webhook')}"></label><div class="wf-actions"><button type="submit">Save Settings</button><button type="button" id="wfSync">Sync Meta Templates</button></div></form></section>`;
}
function rulesHtml(d:Any){ return `<section class="wf-card"><h3>Notification Rules</h3><p>Free-form untuk 24H window open, Meta template untuk selepas 24H.</p><div class="wf-fieldchips">${FIELDS.map(f=>`<code>{${f}}</code>`).join('')}</div></section>${d.rules.map((r:Any)=>ruleCard(r,d.templates)).join('')}`; }
function ruleCard(r:Any,templates:Any[]){ const params=arr(r.template_params).join(', '), fields=arr(r.available_fields).join(', '); return `<section class="wf-card wf-rule"><h3>${esc(r.label)} <small>${esc(r.event_type)}</small></h3><form class="wf-rule-form"><input type="hidden" name="event_type" value="${esc(r.event_type)}"><label class="check"><input type="checkbox" name="enabled" ${r.enabled?'checked':''}> Enable</label><label class="check"><input type="checkbox" name="freeform_enabled" ${r.freeform_enabled?'checked':''}> Free-form when 24H open</label><label class="check"><input type="checkbox" name="template_enabled" ${r.template_enabled?'checked':''}> Template after 24H</label><label class="wide">Free-form message<textarea name="freeform_text" rows="5">${esc(r.freeform_text)}</textarea></label><label>Template<select name="template_name">${templateOptions(templates,r.template_name)}</select>${badge(templates,r.template_name)}</label><label>Language<input name="template_language" value="${esc(r.template_language || 'ms')}"></label><label>Template params order<input name="template_params" value="${esc(params)}"></label><label>Available fields<input name="available_fields" value="${esc(fields)}"></label><div class="wf-preview"><b>Preview</b><pre>${esc(renderText(r.freeform_text,SAMPLE))}</pre></div><div class="wf-actions"><button type="submit">Save Rule</button></div></form></section>`; }
function templatesHtml(d:Any){ return `<section class="wf-card"><h3>Meta Template Guide</h3><p>Create manual dalam Meta/WasapFlow. Lepas approved, tekan Sync Meta Templates.</p><button type="button" id="wfSync2">Sync Meta Templates</button></section><section class="wf-template-grid">${BLUEPRINTS.map(b=>`<article class="wf-card"><h3>${esc(b.name)} ${badge(d.templates,b.name)}</h3><p><b>Category:</b> ${esc(b.category)}<br><b>Event:</b> ${esc(b.event_type)}</p><pre>${esc(b.body)}</pre><p><b>Params:</b> ${b.params.map((p,i)=>`{{${i+1}}}=${p}`).join(', ')}</p><div class="wf-actions"><button type="button" data-copy-template="${esc(b.name)}">Copy Body</button><button type="button" data-copy-json="${esc(b.name)}">Copy JSON</button></div></article>`).join('')}</section>`; }
function testHtml(d:Any){ return `<section class="wf-card"><h3>Send Test</h3><p><b>Auto</b> tidak hantar mode ke backend. Backend akan decide: 24H open = free-form, expired = template.</p><form id="wfSendTest" class="wf-form"><label>Phone<input name="phone" value="${esc(getDraft('test_phone'))}" placeholder="60129554732" required></label><label>Event<select name="event_type">${d.rules.map((r:Any)=>`<option value="${esc(r.event_type)}">${esc(r.label || r.event_type)}</option>`).join('')}</select></label><label>Mode<select name="mode"><option value="auto">Auto 24H decision</option><option value="text">Force free-form</option><option value="template">Force template</option></select></label>${FIELDS.map(k=>`<label>${k}<input name="var_${k}" value="${esc(SAMPLE[k] || '')}"></label>`).join('')}<div class="wf-actions"><button type="submit">Send Test</button></div></form></section>`; }
...
function bind(wrap:HTMLElement,data:Any){
  wrap.querySelectorAll<HTMLInputElement | HTMLSelectElement>('#wfSettings input,#wfSettings select,#wfSendTest input[name="phone"]').forEach(el=>el.addEventListener('input',()=>setDraft(el.name==='phone'?'test_phone':el.name,el.value)));
  const sync = async()=>{ try{ toast('Syncing…'); const r = await edge('whatsapp-admin','/templates/sync',{method:'POST',body:'{}'}); toast(`Synced ${r.synced || 0}`); cache=null; render(wrap,await load(true)); }catch(e:any){ toast(e.message || 'Sync failed', true); } };
  wrap.querySelector('#wfSync')?.addEventListener('click',sync);
  wrap.querySelector('#wfSync2')?.addEventListener('click',sync);
  wrap.querySelector<HTMLFormElement>('#wfSettings')?.addEventListener('submit',async ev=>{ ev.preventDefault(); const f=new FormData(ev.currentTarget); const body:Any={}; ['enabled','base_url','waba_id','default_language','customer_app_base_url','unified_inbox_24h_url'].forEach(k=>body[k]=String(f.get(k)||'')); ['partner_key','webhook_secret','unified_inbox_24h_key'].forEach(k=>{ const v=String(f.get(k)||'').trim(); if(v) body[k]=v; }); try{ await edge('whatsapp-admin','/settings',{method:'POST',body:JSON.stringify(body)}); clearDraft(); cache=null; toast('Settings saved'); render(wrap,await load(true)); }catch(e:any){ toast(e.message || 'Save failed', true); } });
  wrap.querySelectorAll<HTMLFormElement>('.wf-rule-form').forEach(form=>form.addEventListener('submit',async ev=>{ ev.preventDefault(); const f=new FormData(form); const body:Any={ event_type:String(f.get('event_type')), enabled:f.get('enabled')==='on', freeform_enabled:f.get('freeform_enabled')==='on', template_enabled:f.get('template_enabled')==='on', freeform_text:String(f.get('freeform_text')||''), template_name:String(f.get('template_name')||''), template_language:String(f.get('template_language')||'ms'), template_params:String(f.get('template_params')||'').split(',').map(x=>x.trim()).filter(Boolean), available_fields:String(f.get('available_fields')||'').split(',').map(x=>x.trim()).filter(Boolean) }; try{ await edge('whatsapp-admin','/rules',{method:'POST',body:JSON.stringify(body)}); cache=null; toast('Rule saved'); render(wrap,await load(true)); }catch(e:any){ toast(e.message || 'Rule save failed', true); } }));
  wrap.querySelector<HTMLFormElement>('#wfSendTest')?.addEventListener('submit',async ev=>{ ev.preventDefault(); const f=new FormData(ev.currentTarget); const vars:Any={}; Object.keys(SAMPLE).forEach(k=>vars[k]=String(f.get('var_'+k)||SAMPLE[k])); const selectedMode=String(f.get('mode')||'auto'); const payload:Any={ phone:String(f.get('phone')||''), event_type:String(f.get('event_type')||'order_created'), vars, source:'admin_test' }; if(selectedMode !== 'auto') payload.mode = selectedMode; setDraft('test_phone', payload.phone); try{ const r = await edge('whatsapp-send','',{method:'POST',body:JSON.stringify(payload)}); toast(`Sent ${r.mode || selectedMode}: ${r.message_id || 'ok'}`); cache=null; render(wrap,await load(true)); }catch(e:any){ toast(e.message || 'Send failed', true); } });
...
function ensureButton(){
  ...
  btn.id='wfOpenBtn'; btn.textContent='⚡ WhatsApp Control'; btn.onclick=()=>void openPanel();
