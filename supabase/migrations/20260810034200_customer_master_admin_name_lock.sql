...
create or replace function public.icetak_customer_master_preserve_admin_name()
returns trigger
language plpgsql
...
begin
  if nullif(btrim(coalesce(new.admin_name_override,'')),'') is not null then
    ...
    new.display_name:=new.admin_name_override;
    ...
    new.display_name:=old.admin_name_override;
  end if;
  return new;
end;
$$;

drop trigger if exists icetak_customer_master_preserve_admin_name_trg on public.customer_master;
create trigger icetak_customer_master_preserve_admin_name_trg
before insert or update of display_name,admin_name_override on public.customer_master
for each row execute function public.icetak_customer_master_preserve_admin_name();
...
begin
  ...
  if coalesce(p_clear_override,false) then
    ...
    if c.customer_master_id is not null then
      update public.customer_master set admin_name_override=null,admin_name_updated_at=now(),admin_name_updated_by=v_actor,
        metadata=(coalesce(metadata,'{}'::jsonb)-'admin_name_override')||jsonb_build_object('admin_name_updated_by',v_actor,'admin_name_updated_at',now()),updated_at=now()
    ...
    if c.customer_master_id is not null then
      update public.customer_master set admin_name_override=v_name,display_name=v_name,admin_name_updated_at=now(),admin_name_updated_by=v_actor,
        metadata=coalesce(metadata,'{}'::jsonb)||jsonb_build_object('admin_name_override',v_name,'admin_name_updated_by',v_actor,'admin_name_updated_at',now()),updated_at=now()
      where id=c.customer_master_id;
    end if;
  end if;
