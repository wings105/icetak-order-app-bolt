-- Admin V2 Marketplace Orders + Global Search
-- Live production definitions captured after deployment.

CREATE OR REPLACE FUNCTION public.icetak_admin_global_search(p_search text, p_limit integer DEFAULT 8)
 RETURNS jsonb
 LANGUAGE plpgsql
...
begin
  if v_q='' then
    return jsonb_build_object('ok',true,'internalOrders','[]'::jsonb,'marketplaceOrders','[]'::jsonb);
  end if;
  ...
  select coalesce(jsonb_agg(x.obj),'[]'::jsonb) into v_internal
  from (
    select jsonb_build_object(
      'type','internal',
      'orderNo',o.order_no,
      'status',coalesce(o.status,''),
      'customer',coalesce(c.name,o.delivery_name,''),
      'phone',coalesce(c.phone,o.delivery_phone,''),
      'total',coalesce(o.total,0),
      'createdAt',o.created_at
    ) obj
    from public.orders o
    left join public.customers c on c.id=o.customer_id
    where lower(coalesce(o.order_no,'')) like '%'||v_q||'%'
       or lower(coalesce(o.delivery_name,'')) like '%'||v_q||'%'
       or lower(coalesce(c.name,'')) like '%'||v_q||'%'
       or (v_digits<>'' and (
         regexp_replace(coalesce(o.delivery_phone,''),'[^0-9]','','g') like '%'||v_digits||'%'
         or regexp_replace(coalesce(c.phone,''),'[^0-9]','','g') like '%'||v_digits||'%'
       ))
       or lower(coalesce(o.tracking,'')) like '%'||v_q||'%'
    order by o.created_at desc
    limit v_limit
  ) x;

  select coalesce(jsonb_agg(x.obj),'[]'::jsonb) into v_marketplace
  from (
    select jsonb_build_object(
      'type','marketplace',
      'provider',o.provider,
      'orderSn',o.order_sn,
      'status',coalesce(o.current_status,''),
      'buyerUsername',coalesce(o.buyer_username,''),
      'customer',coalesce(nullif(cm.admin_name_override,''),nullif(cm.display_name,''),o.buyer_username,''),
      'phone',coalesce(cm.primary_phone_normalized,''),
      'total',coalesce(fin.buyer_paid,0),
      'createdAt',coalesce(o.placed_at,o.created_at)
    ) obj
    ...
    left join public.marketplace_order_financials fin on fin.order_id=o.id
    where lower(coalesce(o.order_sn,'')) like '%'||v_q||'%'
       ...
       or (v_digits<>'' and regexp_replace(coalesce(cm.primary_phone_normalized,''),'[^0-9]','','g') like '%'||v_digits||'%')
       ...
       or exists (
         ...
         where s.order_id=o.id
           and lower(coalesce(s.tracking_number,'')) like '%'||v_q||'%'
       )
  ...
  return jsonb_build_object(
    'ok',true,
    'internalOrders',v_internal,
    'marketplaceOrders',v_marketplace
  );
end
$function$
;
...
begin
  ...
  with filtered as (
    ...
    where (coalesce(p_provider,'all')='all' or lower(o.provider)=lower(p_provider))
      and (coalesce(p_status,'all')='all' or lower(coalesce(o.current_status,''))=lower(p_status))
      and (
        v_q=''
...
revoke all on function public.icetak_admin_global_search(text,integer) from public, anon;
grant execute on function public.icetak_admin_global_search(text,integer) to authenticated;
