create or replace function public.finance_admin_qrpay_range(p_from date default null::date, p_to date default null::date)
returns jsonb
language sql
...
with source_dates as (
  ...
  select
    ...
    case when p.order_id is not null and o.id is not null then 'matched_order' else 'needs_review' end::text workflow_status,
    p.order_id,o.order_no,o.public_token,
    ...
    case
      when p.order_id is null then coalesce(qc.remark,'Payment received but no real order linked')
      when o.id is null then coalesce(qc.remark,'Payment points to missing order')
      else qc.remark
    end review_remark,
    qc.updated_at review_updated_at,qc.updated_by review_updated_by,
...
select jsonb_build_object(
  ...
  'rows',(select coalesce(jsonb_agg(jsonb_build_object(
    ...
    'whatsapp_link',case when phone is null then null else 'tel:'||phone end,
    'order_link',case when public_token is null then null else 'https://shop.decocake.my/?admin=v2&order='||public_token end,
