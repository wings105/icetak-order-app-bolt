-- Admin V2 Marketplace Orders: shipping priority filters.
-- Adds p_ship_by = all|today|tomorrow using Asia/Kuala_Lumpur local ship-by date.

CREATE OR REPLACE FUNCTION public.icetak_admin_marketplace_orders(p_search text DEFAULT ''::text, p_status text DEFAULT 'all'::text, p_provider text DEFAULT 'all'::text, p_ship_by text DEFAULT 'all'::text, p_limit integer DEFAULT 50, p_offset integer DEFAULT 0)
 RETURNS jsonb
 LANGUAGE plpgsql
...
begin
  ...
  with filtered as (
    ...
    where (coalesce(p_provider,'all')='all' or lower(o.provider)=lower(p_provider))
      and (
        coalesce(p_ship_by,'all')='all'
        or (
          lower(coalesce(p_ship_by,''))='today'
          and (o.ship_by_at at time zone 'Asia/Kuala_Lumpur')::date = (now() at time zone 'Asia/Kuala_Lumpur')::date
        )
        or (
          lower(coalesce(p_ship_by,''))='tomorrow'
          and (o.ship_by_at at time zone 'Asia/Kuala_Lumpur')::date = ((now() at time zone 'Asia/Kuala_Lumpur')::date + 1)
        )
      )
      and (
  ...
  return jsonb_build_object(
    ...
    'summary',jsonb_build_object(
      ...
      'shipByToday',(select count(*) from public.marketplace_orders where current_status in ('READY_TO_SHIP','PROCESSED') and (ship_by_at at time zone 'Asia/Kuala_Lumpur')::date=(now() at time zone 'Asia/Kuala_Lumpur')::date),
      'shipByTomorrow',(select count(*) from public.marketplace_orders where current_status in ('READY_TO_SHIP','PROCESSED') and (ship_by_at at time zone 'Asia/Kuala_Lumpur')::date=((now() at time zone 'Asia/Kuala_Lumpur')::date+1))
    )
  );
end
$function$
...
revoke all on function public.icetak_admin_marketplace_orders(text,text,text,text,integer,integer) from public, anon;
grant execute on function public.icetak_admin_marketplace_orders(text,text,text,text,integer,integer) to authenticated;
