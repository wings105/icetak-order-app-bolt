...
declare
  ...
  v_product_id uuid; v_variant_id uuid; v_catalog_slug text; v_catalog_clickup_task_id text; v_wording_mode text; v_customization jsonb; v_product_snapshot jsonb;
begin
  ...
  for v_item in select * from jsonb_array_elements(v_items) loop
    v_k:=coalesce(v_item->>'k',v_item->>'kind',v_item->>'product_type',v_item->>'productType','edible');
    v_title:=coalesce(v_item->>'title',v_item->>'name','Item');
    ...
    v_product_id:=case when coalesce(v_item->>'product_id','') ~* '^[0-9a-f-]{36}$' then (v_item->>'product_id')::uuid else null end;
    v_variant_id:=case when coalesce(v_item->>'product_variant_id','') ~* '^[0-9a-f-]{36}$' then (v_item->>'product_variant_id')::uuid else null end;
    v_catalog_slug:=nullif(v_item->>'catalog_slug','');
    v_catalog_clickup_task_id:=nullif(v_item->>'catalog_clickup_task_id','');
    v_wording_mode:=nullif(v_item->>'wording_mode','');
    v_customization:=coalesce(v_item->'customization','{}'::jsonb);
    v_product_snapshot:=coalesce(v_item->'product_snapshot','{}'::jsonb);
    insert into public.order_items(
      order_id,order_token,product_type,k,title,qty,price,size,style,wording,custom_text,review_required,workflow,
      product_id,product_variant_id,catalog_slug,catalog_clickup_task_id,wording_mode,customization,product_snapshot
    ) values(
      v_order_db_id,v_public_token,v_k,v_k,v_title,coalesce(nullif(v_item->>'qty','')::int,1),coalesce(nullif(v_item->>'price','')::numeric,0),coalesce(v_item->>'size',''),coalesce(v_item->>'style',''),coalesce(v_item->>'customText',v_item->>'custom_text',v_item->>'wording',''),coalesce(v_item->>'customText',v_item->>'custom_text',v_item->>'wording',''),v_review_required,'Order Received',
      v_product_id,v_variant_id,v_catalog_slug,v_catalog_clickup_task_id,v_wording_mode,v_customization,v_product_snapshot
    ...
    if v_k='burnaway' then
      ...
      values(v_order_db_id,v_item_id,v_public_token,v_item_id::text,'edible','Edible Layer','Order Received',v_review_required,case when v_review_required then 'pending' else 'not_required' end),(v_order_db_id,v_item_id,v_public_token,v_item_id::text,'wafer','Wafer Layer','Order Received',v_review_required,case when v_review_required then 'pending' else 'not_required' end);
    else
      insert into public.production_components(order_id,order_item_id,order_token,item_id,component_type,label,workflow,review_required,review_status)
      values(v_order_db_id,v_item_id,v_public_token,v_item_id::text,v_k,
        case when v_catalog_slug is not null then v_title else case v_k when 'edible' then 'Edible Image' when 'wafer' then 'Wafer Paper' when 'printed' then 'Cake Topper' when 'mirror' then 'Mirror Gold Topper' when 'acrylic' then 'Acrylic Topper' else v_title end end,
        'Order Received',v_review_required,case when v_review_required then 'pending' else 'not_required' end);
    end if;
  end loop;
