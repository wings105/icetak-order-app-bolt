-- Keep customer checkout pricing snapshots and payment reservations in sync when
-- the customer changes courier. A stale active payment session must never remain
-- eligible for matching after the draft amount or delivery method changes.
...
begin
  ...
  -- Persist one canonical total everywhere consumed by review messages, order
  -- finalization, and payment matching.
  ...
  -- Retire any reservation made against the previous courier/amount before the
  -- checkout endpoint prepares a fresh session for the new draft version.
  ...
  where draft_id=d.id
    ...
    and (
      ...
      or coalesce(ps.delivery_code,'') is distinct from delivery_code
