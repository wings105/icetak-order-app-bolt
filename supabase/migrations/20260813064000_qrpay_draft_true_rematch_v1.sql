-- True wrong-customer rematch for QRPay AI drafts.
-- Admin rejects the current conversation -> job returns to retry with that
-- conversation excluded -> worker chooses another candidate -> the SAME draft
-- is refreshed while the previous AI attempt remains in qrpay_order_draft_events.

create or replace function public.icetak_mark_qrpay_draft_needs_rematch(
  p_review_token text,
...
begin
  ...
  v_excluded:=case when jsonb_typeof(coalesce(v_job.evidence->'excluded_conversation_ids','[]'::jsonb))='array' then coalesce(v_job.evidence->'excluded_conversation_ids','[]'::jsonb) else '[]'::jsonb end;
  if v_draft.conversation_id is not null and not (v_excluded ? v_draft.conversation_id::text) then v_excluded:=v_excluded||jsonb_build_array(v_draft.conversation_id::text); end if;

  update public.qrpay_order_drafts set status='needs_rematch',admin_link_sent_at=null,updated_at=now() where id=v_draft.id;
  insert into public.qrpay_order_draft_events(draft_id,event_type,actor,before_data,metadata)
  values(v_draft.id,'admin_requested_rematch',coalesce(nullif(p_actor,''),'admin-link'),v_draft.working_draft,jsonb_build_object('excluded_conversation_id',v_draft.conversation_id));
  update public.admin_order_reviews set status='awaiting_admin_detail',fallback_notified_at=null,last_notified_at=null,updated_at=now() where draft_id=v_draft.id;
  update public.qrpay_ai_jobs set
    status='retry',next_attempt_at=now(),completed_at=null,locked_at=null,last_error=null,
    matched_conversation_id=null,matched_phone=null,matched_bsuid=null,matched_username=null,matched_customer_master_id=null,matched_customer_name=null,
    match_score=null,match_reason='admin_wrong_customer_retry',extraction='{}'::jsonb,
    evidence=jsonb_set(coalesce(evidence,'{}'::jsonb),'{excluded_conversation_ids}',v_excluded,true),updated_at=now()
  where id=v_draft.qrpay_job_id and order_id is null;
  return jsonb_build_object('success',true,'draft_id',v_draft.id,'status','needs_rematch','job_status','retry','excluded_conversation_ids',v_excluded);
end;
$$;

create or replace function public.icetak_create_or_update_qrpay_draft(
  p_job_id uuid,
  p_payload jsonb,
...
begin
  ...
  if found and v_existing.status<>'needs_rematch' then
    return jsonb_build_object('success',true,'duplicate',true,'draft_id',v_existing.id,'review_token',v_existing.review_token,'status',v_existing.status,'transaction_id',v_existing.transaction_id);
  end if;

  if found then
    v_event_type:='ai_rematch_draft_created';
    insert into public.qrpay_order_draft_events(draft_id,event_type,actor,before_data,after_data,metadata)
    values(v_existing.id,v_event_type,'qrpay-ai-worker',v_existing.ai_draft,v_work,jsonb_build_object('previous_conversation_id',v_existing.conversation_id,'new_conversation_id',v_job.matched_conversation_id,'worker_version',v_worker_version));
    update public.qrpay_order_drafts set
      ...
      customer_name=coalesce(nullif(p_payload#>>'{customer,name}',''),v_job.matched_customer_name),
      match_score=coalesce(v_job.match_score,nullif(p_payload->>'match_score','')::numeric),match_reason=coalesce(v_job.match_reason,p_payload->>'match_reason'),
  ...
  return jsonb_build_object('success',true,'draft_created',true,'rematched',v_event_type='ai_rematch_draft_created','draft_id',v_draft.id,'review_token',v_draft.review_token,'admin_review_id',v_review_id,'transaction_id',v_draft.transaction_id,'payment_amount',v_draft.payment_amount,'draft_total',v_draft.draft_total,'payment_difference',v_draft.payment_difference,'status',v_draft.status);
end;
$$;

revoke all on function public.icetak_mark_qrpay_draft_needs_rematch(text,text),public.icetak_create_or_update_qrpay_draft(uuid,jsonb,text) from public,anon,authenticated;
grant execute on function public.icetak_mark_qrpay_draft_needs_rematch(text,text),public.icetak_create_or_update_qrpay_draft(uuid,jsonb,text) to service_role;
