...
async function sendLoginMessage(phone: string, customerName: string, otp: string, magicLink: string) {
  const response = await fetch(`${SUPABASE_URL}/functions/v1/whatsapp-send`, {
    ...
    body: JSON.stringify({ phone, event_type: "customer_login", customer_name: customerName, vars: { customer_name: customerName, otp, magic_link: magicLink } }),
  });
...
Deno.serve(async (req) => {
  try {
    ...
    if (path === "/request") {
      ...
      const send = await sendLoginMessage(phone, customerName, otp, magicLink);
      return json({ ok: true, phone, sent: send.ok === true, expires_in_seconds: 600 });
    }
