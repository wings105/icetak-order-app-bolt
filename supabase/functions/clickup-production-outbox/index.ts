...
function links(base:string,order:any,componentId?:string){const token=encodeURIComponent(text(order.public_token)),component=componentId?`&component=${encodeURIComponent(componentId)}`:'',hash=componentId?`#component-${encodeURIComponent(componentId)}`:'',customer=`/?order=${token}${hash}`,admin=`/?admin=v2&order=${token}${component}`,history=`/?c=${encodeURIComponent(text(order.customer_token))}`;return{customer_order_path:customer,admin_order_path:admin,customer_history_path:history,customer_order_link:base?`${base}${customer}`:null,admin_order_link:base?`${base}${admin}`:null,customer_history_link:base?`${base}${history}`:null}}
