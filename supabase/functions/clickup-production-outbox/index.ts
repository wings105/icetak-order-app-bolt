...
async function integrationSettings() {
  const [{ data: clickup, error }, { data: app }] = await Promise.all([
    ...
    db.from('system_settings').select('value').eq('key', 'public_app').maybeSingle(),
  ]);
  if (error) throw error;
  return {
    clickup: clickup?.value || {},
    baseUrl: trimSlash(text(app?.value?.base_url) || 'https://8ab71fa9c33743fd70.v2.appdeploy.ai'),
  };
}
...
function links(baseUrl: string, order: any, componentId?: string) {
  ...
  return {
    customer_order_link: `${baseUrl}/?order=${orderToken}${hash}`,
    admin_order_link: `${baseUrl}/?admin=1&order=${orderToken}${component}`,
    customer_history_link: `${baseUrl}/?c=${encodeURIComponent(text(order.customer_token))}`,
  };
}
function componentDescription(baseUrl: string, order: any, component: any, item: any) {
  ...
  const lines = [
    ...
    `System Link: ${componentLinks.admin_order_link}`,
    `Customer Link: ${componentLinks.customer_order_link}`,
  ];
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    for (const event of events || []) {
      ...
      if (orderError) {
        await db.from('integration_outbox').update({ status: 'retry', last_error: orderError.message, next_attempt_at: new Date(Date.now() + 60_000).toISOString(), locked_at: null }).eq('id', event.id);
        continue;
      }
      ...
      if (!(components || []).length) {
        await db.from('integration_outbox').update({
          status: 'processed', processed_at: new Date().toISOString(), sent_at: new Date().toISOString(),
          locked_at: null, last_error: null, error: null,
        }).eq('id', event.id);
        continue;
      }
    ...
    return json({ ok: true, mode: settings.clickup?.mode || 'observe', count: results.length, events: results });
  } catch (error) {
