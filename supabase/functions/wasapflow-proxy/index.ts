...
async function templateMeta(name: string, language: string) {
  const response = await fetch(`${U}/rest/v1/whatsapp_templates?name=eq.${encodeURIComponent(name)}&language=eq.${encodeURIComponent(language)}&status=eq.APPROVED&select=category,components&limit=1`, {
    headers: { apikey: K, authorization: `Bearer ${K}` },
  });
  const rows = await response.json().catch(() => []);
  return rows?.[0] || null;
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!['/messages/send', '/messages/template', '/messages/interactive'].includes(pathname)) {
    ...
    const upstream = await fetchTimed(`${OFFICIAL}${pathname}`, {
      method: 'POST',
    ...
    return new Response(JSON.stringify({
      success: false,
      error: timeout ? 'WasapFlow upstream timeout' : error instanceof Error ? error.message : String(error),
    }), {
