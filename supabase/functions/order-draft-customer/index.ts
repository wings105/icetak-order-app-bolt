...
const meaningful=(v:any,min=2)=>t(v).replace(/[^\p{L}\p{N}]/gu,'').length>=min;
const validAddress=(c:any)=>meaningful(c?.address_line1,3)&&/^\d{5}$/.test(digits(c?.postcode))&&meaningful(c?.city,2)&&meaningful(c?.state,2);
function sanitizeDraftAddress(work:any){
  const w=structuredClone(work||{}),c={...(w.customer||{})};
  if(String(w.delivery||'').toLowerCase()!=='pickup'&&!validAddress(c)){
    if(!meaningful(c.address_line1,3))c.address_line1='';
    if(!/^\d{5}$/.test(digits(c.postcode)))c.postcode='';
    if(!meaningful(c.city,2))c.city='';
    if(!meaningful(c.state,2))c.state='';
    c.address_id=null;
    w.customer=c;
    delete w.address_id;
  }
...
async function savedAddresses(d:any){
  try{
    ...
    if(!masterId){const iq=await db.from('customer_identifiers_master').select('customer_master_id').eq('identifier_type','phone').eq('normalized_value',p).eq('scope','global').limit(1);masterId=iq.data?.[0]?.customer_master_id||null;}
    ...
    if(c&&masterId)q=q.or(`customer_id.eq.${c.id},customer_master_id.eq.${masterId}`);else if(masterId)q=q.eq('customer_master_id',masterId);else q=q.eq('customer_id',c.id);
    ...
    return a.error?[]:(a.data||[]).filter(validAddress);
  }catch{return[]}
}
async function load(token:string){
  ...
  const d={...q.data,working_draft:sanitizeDraftAddress(q.data.working_draft)};let ps:any=null,order:any=null;
...
Deno.serve(async req=>{
  ...
  try{
    ...
    if(body.action==='confirm'){
      ...
      if(String(d.working_draft?.delivery||'').toLowerCase()!=='pickup'&&!validAddress(c))return out({ok:false,error:'Sila lengkapkan alamat, poskod, bandar dan negeri sebelum bayar'},400);
      const line1=t(c.address_line1),line2=t(c.address_line2),allowed:any={name:t(c.name)||undefined,phone:t(c.phone)||undefined,address_line1:line1?(line2?`${line1}, ${line2}`:line1):undefined,address_line2:line2||undefined,postcode:digits(c.postcode)||undefined,city:t(c.city)||undefined,state:t(c.state)||undefined};
