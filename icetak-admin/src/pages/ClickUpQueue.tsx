  ...
  return <div className="cuq-page fade-in">
    ...
    <div className="panel cuq-panel">
      ...
      <div className="table-wrap">
        {loading?<div className="loading"><span className="spinner"/> Loading ClickUp queue...</div>:rows.length===0?<div className="empty"><div className="empty-title">Queue clear</div><p>No records for this filter.</p></div>:<table className="cuq-table"><thead><tr><th>Order</th><th>Queue</th><th>Components</th><th>ClickUp</th><th>Timing</th><th>Reason / Error</th><th>Action</th></tr></thead><tbody>{rows.map(row=><QueueTableRow key={row.orderDbId} row={row} busy={busy===row.orderDbId} canEdit={canEdit} onView={()=>{setDetailRef(row.orderDbId);setDetail(null);}} onRetry={()=>void retry(row)} onOpenOrder={()=>onOpenOrder?.(row.orderNo)}/>)}</tbody></table>}
      </div>
...
function QueueTableRow({row,busy,canEdit,onView,onRetry,onOpenOrder}:{row:QueueRow;busy:boolean;canEdit:boolean;onView:()=>void;onRetry:()=>void;onOpenOrder:()=>void}){
  ...
  return <tr>
    ...
    <td><div className="cuq-tasklinks">{(row.components||[]).filter(c=>c.clickupTaskId).slice(0,3).map(c=><a key={c.componentId} href={c.taskUrl||`https://app.clickup.com/t/3747262/${c.clickupTaskId}`} target="_blank" rel="noreferrer">{c.clickupTaskId}</a>)}{!(row.components||[]).some(c=>c.clickupTaskId)&&<span className="cuq-sub">{held?'Waiting release':'No task ID yet'}</span>}</div></td>
