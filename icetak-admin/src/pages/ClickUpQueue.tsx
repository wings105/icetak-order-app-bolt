...
type QueueRow = {
  ...
  outboxId?:string|null; outboxStatus?:string; status:string; attempts?:number; lastError?:string; holdReason?:string|null; productionReady?:boolean;
  queuedAt?:string|null; lockedAt?:string|null; nextAttemptAt?:string|null; processedAt?:string|null;
...
type Detail = {
  order?: { id?:string; orderNo?:string; customerName?:string; customerPhone?:string; dateNeed?:string; source?:string; status?:string; adminStatus?:string; productionApproved?:boolean; productionReady?:boolean; holdReason?:string|null; publicToken?:string; adminOrderLink?:string };
...
export default function ClickUpQueue({ permissions=[], onOpenOrder }:Props){
  ...
  const canApprove=permissions.includes('approve_production');
  ...
  const confirmProduction=async(row:Pick<QueueRow,'orderDbId'|'orderNo'>)=>{
    if(!canApprove){setError('Permission approve_production required.');return;}
    if(!window.confirm(`Confirm Production untuk ${row.orderNo} sebagai admin? Ini akan override customer confirmation jika perlu dan release ClickUp Queue.`)) return;
    setBusy(row.orderDbId);setError('');
    const {error:rpcError}=await supabase.rpc('icetak_admin_confirm_production',{p_order_id:row.orderDbId});
    setBusy('');
    if(rpcError){setError(rpcError.message);return;}
    setNotice(`${row.orderNo}: production confirmed · ClickUp Queue released.`);
    await load();if(detailRef)await loadDetail(row.orderDbId);
  };
  ...
  return <div className="cuq-page fade-in">
    ...
    <div className="cuq-metrics">
      <Metric label="Attention" value={summary.attention} tone={Number(summary.attention||0)>0?'danger':'neutral'}/>
      <Metric label="Processing" value={summary.processing} tone="info"/>
      <Metric label="Waiting" value={summary.waiting} tone="neutral"/>
      <Metric label="Held" value={summary.held} tone={Number(summary.held||0)>0?'warning':'neutral'}/>
      <Metric label="Retry" value={summary.retrying} tone="warning"/>
      <Metric label="Failed" value={summary.failed} tone={Number(summary.failed||0)>0?'danger':'neutral'}/>
      <Metric label="Success Today" value={summary.successToday} tone="success"/>
    </div>
    ...
    <div className="panel cuq-panel">
      ...
      <div className="cuq-toolbar"><input value={query} onChange={e=>{setQuery(e.target.value);setPage(1);}} placeholder="Search order, customer, component, reason, error..."/><span className="cuq-live">Auto refresh 30s</span></div>
      <div className="table-wrap">
        {loading?<div className="loading"><span className="spinner"/> Loading ClickUp queue...</div>:rows.length===0?<div className="empty"><div className="empty-title">Queue clear</div><p>No records for this filter.</p></div>:<table className="cuq-table"><thead><tr><th>Order</th><th>Queue</th><th>Components</th><th>ClickUp</th><th>Timing</th><th>Reason / Error</th><th>Action</th></tr></thead><tbody>{rows.map(row=><QueueTableRow key={row.orderDbId} row={row} busy={busy===row.orderDbId} canEdit={canEdit} canApprove={canApprove} onView={()=>{setDetailRef(row.orderDbId);setDetail(null);}} onRetry={()=>void retry(row)} onConfirm={()=>void confirmProduction(row)} onOpenOrder={()=>onOpenOrder?.(row.orderNo)}/>)}</tbody></table>}
      </div>
    ...
    {detailRef&&<QueueDrawer detail={detail} loading={detailLoading} fallback={selected} busy={busy} canEdit={canEdit} canApprove={canApprove} onClose={()=>{setDetailRef('');setDetail(null);}} onRetry={()=>detail?.order?.id&&void retry({orderDbId:detail.order.id,orderNo:detail.order.orderNo||detail.order.id})} onConfirm={()=>detail?.order?.id&&void confirmProduction({orderDbId:detail.order.id,orderNo:detail.order.orderNo||detail.order.id})} onLink={c=>void linkExisting(c)} onCopyPayload={()=>void copyPayload()} onOpenOrder={()=>detail?.order?.orderNo&&onOpenOrder?.(detail.order.orderNo)}/>} 
  </div>;
}

function QueueTableRow({row,busy,canEdit,canApprove,onView,onRetry,onConfirm,onOpenOrder}:{row:QueueRow;busy:boolean;canEdit:boolean;canApprove:boolean;onView:()=>void;onRetry:()=>void;onConfirm:()=>void;onOpenOrder:()=>void}){
  const meta=statusMeta(row.status);
  const held=row.status==='held';
  const retryable=['failed','stale','data_problem','missing_queue','partial'].includes(row.status);
  const reason=row.holdReason||row.lastError||'—';
  return <tr>
    <td><button className="cuq-order" onClick={onView}>{row.orderNo}</button><div className="cuq-sub">{row.customerName||'—'} · {row.customerPhone||'—'}</div><div className="cuq-sub">Need {date(row.dateNeed)} · {row.source||'—'}</div></td>
    <td><span className={`cuq-status ${meta.tone}`}>{meta.label}</span><div className="cuq-sub">{held?'Not released to AP':`Outbox: ${row.outboxStatus||'—'} · attempt ${row.attempts||0}`}</div></td>
    <td><b>{row.componentsLinked||0}/{row.componentsTotal||0} linked</b><div className="cuq-sub">{row.componentSummary||'—'}</div>{Number(row.componentsMissing||0)>0&&<span className="cuq-missing">{row.componentsMissing} missing</span>}</td>
    <td><div className="cuq-tasklinks">{(row.components||[]).filter(c=>c.clickupTaskId).slice(0,3).map(c=><a key={c.componentId} href={c.taskUrl||`https://app.clickup.com/t/3747262/${c.clickupTaskId}`} target="_blank" rel="noreferrer">{c.clickupTaskId}</a>)}{!(row.components||[]).some(c=>c.clickupTaskId)&&<span className="cuq-sub">{held?'Waiting admin confirmation':'No task ID yet'}</span>}</div></td>
    <td><div>Queued {dt(row.queuedAt)}</div><div className="cuq-sub">Claimed {dt(row.lockedAt)}</div><div className="cuq-sub">Next {dt(row.nextAttemptAt)}</div></td>
    <td className="cuq-errorcell">{reason}</td>
    <td><div className="cuq-actions"><button className="btn btn-outline btn-sm" onClick={onView}>View</button>{held?<><button className="btn btn-primary btn-sm" disabled={busy||!canApprove} onClick={onConfirm}>{busy?'Working…':'Confirm Production'}</button><button className="btn btn-ghost btn-sm" onClick={onOpenOrder}>Fix Order</button></>:retryable&&<button className="btn btn-primary btn-sm" disabled={busy||!canEdit} onClick={onRetry}>{busy?'Working…':'Retry Missing'}</button>}{!held&&<button className="btn btn-ghost btn-sm" onClick={onOpenOrder}>Order</button>}</div></td>
  </tr>;
}

function QueueDrawer({detail,loading,fallback,busy,canEdit,canApprove,onClose,onRetry,onConfirm,onLink,onCopyPayload,onOpenOrder}:{detail:Detail|null;loading:boolean;fallback?:QueueRow;busy:string;canEdit:boolean;canApprove:boolean;onClose:()=>void;onRetry:()=>void;onConfirm:()=>void;onLink:(c:DetailComponent)=>void;onCopyPayload:()=>void;onOpenOrder:()=>void}){
  const [tab,setTab]=useState<'overview'|'payload'|'timeline'>('overview');
  const order=detail?.order;
  const outbox=detail?.outbox;
  const held=order?.productionReady===false;
  const retryable=!held&&['failed','stale','data_problem','missing_queue','partial'].includes(String(fallback?.status||''));
  return <div className="cuq-overlay" onMouseDown={e=>{if(e.target===e.currentTarget)onClose();}}><aside className="cuq-drawer">{loading||!detail?<div className="loading"><span className="spinner"/> Loading queue detail...</div>:<><header><div><div className="page-label">ClickUp Queue Detail</div><h2>{order?.orderNo||fallback?.orderNo}</h2><div className="cuq-sub">{order?.customerName||fallback?.customerName} · {order?.customerPhone||fallback?.customerPhone}</div></div><button onClick={onClose}>×</button></header><nav><button className={tab==='overview'?'active':''} onClick={()=>setTab('overview')}>Overview</button><button className={tab==='payload'?'active':''} onClick={()=>setTab('payload')}>AP Payload</button><button className={tab==='timeline'?'active':''} onClick={()=>setTab('timeline')}>Timeline</button></nav><div className="cuq-drawerbody">
    {tab==='overview'&&<><section className="cuq-card"><h3>Queue State</h3>{held&&<div style={{marginBottom:12,padding:'10px 12px',borderRadius:10,background:'#fff7ed',color:'#9a3412',fontWeight:700}}>HELD — {order?.holdReason||fallback?.holdReason||'Order belum production-ready'}. AP belum menerima job ini.</div>}<div className="cuq-kvs"><KV k="Production Ready" v={held?'No':'Yes'}/><KV k="Hold Reason" v={order?.holdReason||fallback?.holdReason||'—'}/><KV k="Outbox" v={outbox?.status||'—'}/><KV k="Attempts" v={String(outbox?.attempts||0)}/><KV k="Queued" v={dt(outbox?.queuedAt)}/><KV k="Claimed" v={dt(outbox?.lockedAt)}/><KV k="Next Attempt" v={dt(outbox?.nextAttemptAt)}/><KV k="Last Error" v={outbox?.lastError||'—'}/></div><div className="cuq-actions">{held&&canApprove&&<button className="btn btn-primary btn-sm" disabled={Boolean(busy)} onClick={onConfirm}>{busy?'Working…':'Confirm Production'}</button>}<button className="btn btn-outline btn-sm" onClick={onOpenOrder}>{held?'Fix Order':'Open Order'}</button>{canEdit&&retryable&&<button className="btn btn-primary btn-sm" disabled={Boolean(busy)} onClick={onRetry}>Retry Missing</button>}</div></section><section className="cuq-card"><h3>Components</h3>{(detail.components||[]).map(c=><div className="cuq-component" key={c.componentId}><div><b>{c.label||'Component'}</b><span>{c.item?.title||c.type||'—'} · {c.item?.size||'—'} · {c.item?.style||'—'} · {money(c.item?.price)}</span><span>Component {c.componentId}</span></div><div>{c.clickupTaskId?<><span className="cuq-status success">LINKED</span><a href={c.taskUrl||`https://app.clickup.com/t/3747262/${c.clickupTaskId}`} target="_blank" rel="noreferrer">{c.clickupTaskId}</a></>:<><span className="cuq-status warning">NOT LINKED</span>{held?<span className="cuq-sub">Waiting admin confirmation</span>:canEdit&&<button className="btn btn-outline btn-sm" disabled={busy===c.componentId} onClick={()=>onLink(c)}>Link Existing</button>}</>}</div></div>)}</section></>}
