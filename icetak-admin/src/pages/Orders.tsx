...
function AdvancedFilters({ filters, onChange, onClear }: { filters: Filters; onChange: (key: keyof Filters, value: string) => void; onClear: () => void }) {
  return <div className="erp-advanced-filters">
    ...
    <FilterField label="Delivery"><select value={filters.delivery || ''} onChange={(e) => onChange('delivery', e.target.value)}><option value="">All</option><option value="pickup">Pickup</option><option value="spx">SPX</option><option value="j&t">J&T</option><option value="jnt">JNT</option></select></FilterField>
  ...
  return <div className="erp-drawer-overlay" onMouseDown={(e) => { if (e.target === e.currentTarget) onClose(); }}><aside className="erp-order-drawer">
    {loading || !order ? <div className="loading"><span className="spinner" /> Loading order detail...</div> : <>
      ...
      <div className="erp-drawer-body">
        ...
        {tab === 'items' && <div>{items.map((item, index) => <div className="erp-drawer-card erp-item-editor" key={item.id}><h3>{index + 1}. {item.title || item.k || 'Item'}</h3><div className="cell-sub">{item.size || '—'} · {item.style || '—'} · {item.workflow || '—'}</div><div className="erp-item-fields"><FilterField label="Qty"><input type="number" min="1" disabled={!canEdit} value={item.qty || 1} onChange={(e) => setItems((old) => old.map((x) => x.id === item.id ? { ...x, qty: Math.max(1, Number(e.target.value || 1)) } : x))} /></FilterField><FilterField label="Unit Price"><input type="number" min="0" step="0.01" disabled={!canEdit} value={item.price || 0} onChange={(e) => setItems((old) => old.map((x) => x.id === item.id ? { ...x, price: Math.max(0, Number(e.target.value || 0)) } : x))} /></FilterField><FilterField label="Custom Text"><input disabled={!canEdit} value={item.customText || ''} onChange={(e) => setItems((old) => old.map((x) => x.id === item.id ? { ...x, customText: e.target.value } : x))} /></FilterField>{item.reviewRequired && <FilterField label="Design Preview URL"><input disabled={!canEdit} value={item.previewUrl || ''} onChange={(e) => setItems((old) => old.map((x) => x.id === item.id ? { ...x, previewUrl: e.target.value } : x))} /></FilterField>}</div>{(item.components || []).map((c) => <div className="erp-component-line" key={c.id}><div><b>{c.label || 'Component'}</b><span>{c.customerLabel || c.workflow || '—'} · {c.reviewStatus || '—'}</span></div><div>{c.clickupTaskId ? <a className="erp-inline-link" href={`https://app.clickup.com/t/3747262/${c.clickupTaskId}`} target="_blank" rel="noreferrer">ClickUp {c.clickupTaskId}</a> : 'Not linked'}</div></div>)}</div>)}{canEdit && <div className="erp-drawer-save"><button className="btn btn-primary" disabled={saving} onClick={() => void save()}>{saving ? 'Saving…' : 'Save Item Changes'}</button></div>}</div>}
