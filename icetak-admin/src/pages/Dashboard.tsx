...
function OrderCard({ order, onOpen }: { order: EnterpriseOrder; onOpen: () => void }) {
  ...
  return <div className="order-card">
    ...
    <div className="order-customer"><span className="order-customer-name">{order.customerName || 'Customer'}</span>{normalizedPhone ? <a className="order-customer-phone" href={`https://wa.me/${normalizedPhone}`} target="_blank" rel="noreferrer">{phone}</a> : <span className="order-customer-phone">—</span>}</div>
