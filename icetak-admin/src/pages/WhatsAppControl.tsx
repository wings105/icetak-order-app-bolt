...
function RuleCard({ rule, templates, onSaved }: { rule: AnyRow; templates: AnyRow[]; onSaved: () => Promise<void> }) {
  ...
  const isPickupPaymentRule=rule.event_type==='order_ready_pickup_auto';
  const [form, setForm] = useState({ enabled: Boolean(rule.enabled), freeform_enabled: Boolean(rule.freeform_enabled), template_enabled: Boolean(rule.template_enabled), freeform_text: String(rule.freeform_text || ''), freeform_text_paid:String(rule.freeform_text_paid || ''), freeform_text_unpaid:String(rule.freeform_text_unpaid || ''), template_name: String(rule.template_name || ''), template_language: String(rule.template_language || 'ms'), template_params: asArray(rule.template_params?.value || rule.template_params).join(', ') });
  const [busy, setBusy] = useState(false); const [error, setError] = useState<string | null>(null);
  const save = async () => { setBusy(true); setError(null); const payload:AnyRow = { event_type: rule.event_type, enabled: form.enabled, freeform_enabled: form.freeform_enabled, template_enabled: form.template_enabled, freeform_text: form.freeform_text, template_name: form.template_name, template_language: form.template_language, template_params: asArray(form.template_params), available_fields: FIELDS }; if(isPickupPaymentRule){payload.freeform_text_paid=form.freeform_text_paid;payload.freeform_text_unpaid=form.freeform_text_unpaid;} const { error: rpcError } = await supabase.rpc('icetak_admin_whatsapp_save_rule', { p_payload: payload }); setBusy(false); if (rpcError) setError(rpcError.message); else await onSaved(); };
  return <details className="panel"><summary style={{ cursor: 'pointer', padding: 16, fontWeight: 800,display:'flex',gap:8,alignItems:'center' }}><span>{rule.label || rule.event_type} · {form.enabled ? 'Enabled' : 'Disabled'}</span><span className={`badge ${state.code==='ready'?'badge-success':state.code==='bad'?'badge-error':state.code==='freeform'?'badge-warning':'badge-neutral'}`} style={{marginLeft:'auto'}} title={state.detail}>{state.label}</span></summary><div style={{ padding: '0 18px 18px', display: 'grid', gap: 10 }}>{error && <div style={{ color:'#b42318' }}>{error}</div>}<div className="cell-sub">{isPickupPaymentRule?'Sistem semak payment sebelum menghantar. Template selepas 24H kekal sama.':state.detail}</div><label><input type="checkbox" checked={form.enabled} onChange={(e) => setForm({ ...form, enabled:e.target.checked })}/> Enable notification</label><label><input type="checkbox" checked={form.freeform_enabled} onChange={(e) => setForm({ ...form, freeform_enabled:e.target.checked })}/> Free-form when 24H open</label><label><input type="checkbox" checked={form.template_enabled} onChange={(e) => setForm({ ...form, template_enabled:e.target.checked })}/> Template after 24H</label>{isPickupPaymentRule?<><Field label="Free-form message · PAID"><textarea rows={7} value={form.freeform_text_paid} onChange={(e) => setForm({ ...form, freeform_text_paid:e.target.value })}/></Field><Field label="Free-form message · UNPAID"><textarea rows={8} value={form.freeform_text_unpaid} onChange={(e) => setForm({ ...form, freeform_text_unpaid:e.target.value })}/></Field></>:<Field label="Free-form message"><textarea rows={5} value={form.freeform_text} onChange={(e) => setForm({ ...form, freeform_text:e.target.value })}/></Field>}<Field label="Approved template"><select value={form.template_name} onChange={(e) => setForm({ ...form, template_name:e.target.value })}><option value="">Select...</option>{templates.map((t) => <option key={`${t.name}-${t.language}`} value={t.name}>{t.name} ({t.language || 'ms'})</option>)}</select></Field><Field label="Language"><input value={form.template_language} onChange={(e) => setForm({ ...form, template_language:e.target.value })}/></Field><Field label="Template params"><input value={form.template_params} onChange={(e) => setForm({ ...form, template_params:e.target.value })}/></Field><div><button className="btn btn-primary" disabled={busy} onClick={() => void save()}>{busy ? 'Saving...' : 'Save Rule'}</button></div></div></details>;
}
