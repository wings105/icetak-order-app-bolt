...
export default function AiDashboard({onOpenOrder,onOpenDrafts,canViewDrafts}:Props){
 ...
 const [training,setTraining]=useState<Data[]>([]),[trainingDirty,setTrainingDirty]=useState(false),[sideTab,setSideTab]=useState<'orders'|'ai'>('orders');
 ...
 const open=async(row:Row)=>{
  ...
  try{const d=await call({action:'detail',conversation_id:row.id});if(seq!==detailSeq.current)return;
   setSelected(d.row);setEvents(d.events||[]);setTraining(d.training||[]);setCaps(d.capabilities);setResponse(d.row.analysis.response_text||'');
 ...
 const choose=(row:Row)=>{if(busy)return;if(hasUnsaved){setPendingRow(row);setCloseConfirm(true);return;}setDetailTab('chat');void open(row);};
 ...
 return <section className={`ai-board ${viewMode==='conversation'?'ai-conversation-home':''}`}>
  <div className="ai-heading"><div><span className="ai-eyebrow">WHATSAPP + SHOPEE</span><h1>{viewMode==='conversation'?'Conversation workspace':'AI Action Dashboard'}</h1><p>Chat, konteks order dan semakan AI dalam satu ruang.</p></div><div className="ai-heading-actions"><span className="ai-mode">Semakan admin • Auto-send OFF</span><button disabled={loading||busy||hasUnsaved} onClick={()=>void load(false)}>↻ Muat semula</button></div></div>
  ...
  <div className="ai-list-meta"><span>{visible.length} perbualan dipaparkan · Susunan dan jumlah berdasarkan data dimuat</span><div className="ai-view-controls"><span>Disemak {date(fetched)} MYT</span><div className="ai-view-toggle" role="group" aria-label="Jenis paparan dashboard"><button type="button" disabled={busy||hasUnsaved} aria-pressed={viewMode==='conversation'} onClick={()=>setViewMode('conversation')}>Conversation</button><button type="button" disabled={busy||hasUnsaved} aria-pressed={viewMode==='card'} onClick={()=>{setViewMode('card');detailSeq.current++;setSelected(null);}}>Card view</button><button type="button" disabled={busy||hasUnsaved} aria-pressed={viewMode==='list'} onClick={()=>{setViewMode('list');detailSeq.current++;setSelected(null);}}>List view</button></div></div></div>
  ...
  {!loading&&!visible.length&&!selected&&<div className="ai-empty"><h2>Tiada kad dalam tapisan ini</h2><p>Tukar tapisan atau muatkan chat seterusnya. Ini bukan pengesahan semua chat sudah selesai.</p><button onClick={()=>{setStatus('');setIntent('');}}>Lihat semua kad dimuat</button></div>}
  {more&&viewMode!=='conversation'&&<div className="ai-more"><button disabled={loading} onClick={()=>void load(true)}>{loading?'Memuatkan…':'Muat 30 chat seterusnya'}</button></div>}
  ...
  {selected&&<div className={viewMode==='conversation'?`ai-inline-workspace ${expanded?'ai-expanded':''}`:'ai-overlay'}><div className={`ai-detail ai-workspace tab-${detailTab}`}  role={viewMode==='conversation'?'region':'dialog'} aria-modal={viewMode==='conversation'?undefined:true} aria-labelledby="ai-detail-name" ref={dialogRef}>
   ...
   {detailLoading?<div className="ai-empty" role="status">Memuatkan bukti dan semakan konteks…</div>:<>
    ...
    </div><aside className="ai-compose"><div className="ai-side-tabs" role="group" aria-label="Panel pelanggan"><button aria-pressed={sideTab==='orders'} onClick={()=>setSideTab('orders')}>Order ({(ctx.orders?.length||0)+(ctx.marketplace_orders?.length||0)})</button><button aria-pressed={sideTab==='ai'} onClick={()=>setSideTab('ai')}>✦ AI & latihan</button></div><div hidden={sideTab!=='orders'}><CustomerOrders key={selected.id} compact context={ctx} onOpenOrder={onOpenOrder}/><button className="ai-primary ai-wide" onClick={()=>setSideTab('ai')}>Semak cadangan AI →</button></div><div hidden={sideTab!=='ai'}><section className="ai-box ai-confidence-panel"><div className="ai-section-title"><h3>✦ {a.intent_label}</h3><span className="ai-confidence">Confidence {a.confidence}</span></div><p className="ai-muted">AI membaca chat → semak konteks → cadangkan balasan. Menunggu keputusan admin.</p><details><summary>Bukti & batasan ({a.warnings?.length||0})</summary><p>{a.confidence_note}</p><small>{a.basis} · {a.engine}</small>{a.confidence_reasons?.map((r:string)=><p key={r}>• {r}</p>)}{a.warnings?.map((w:string)=><p className="ai-warning" key={w}>{w}</p>)}</details></section><section className="ai-box"><h3>Cadangan balasan</h3><p className="ai-muted">Draf SOP untuk disemak. Harga, ukuran dan janji siap mesti disahkan admin.</p><label htmlFor="ai-response">Edit sebelum membalas</label><textarea id="ai-response" value={response} onChange={e=>{setResponse(e.target.value);setDirty(true);sendKey.current=null;}} rows={8} maxLength={4000} disabled={detailFailed||busy||!caps.can_manage}/>
