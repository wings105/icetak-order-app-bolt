    ...
    <div className="fade-in">
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">Message queue and delivery history</p>
        </div>
      ...
      <div className="panel">
        <div className="panel-header">
          <div className="panel-title">Messages ({rows.length})</div>
        </div>
        <div className="table-wrap">
          {loading ? (
            <div className="loading"><span className="spinner" /> Loading...</div>
          ) : rows.length === 0 ? (
            <div className="empty"><div className="empty-title">No messages yet</div></div>
          ) : (
            <table>
              <thead>
                <tr>
                ...
                {rows.map((m) => {
                  ...
                  return (
                    <tr key={m.id} className="row-hover">
                      ...
                      <td><span className="tag tag-neutral">{m.mode || m.message_type || 'text'}</span></td>
                      <td style={{ maxWidth: 280, overflow: 'hidden' }}>
                        <div className="cell-name" style={{ fontSize: 12 }}>{m.template_name || '—'}</div>
                        {m.body && <div className="cell-sub" style={{ whiteSpace: 'nowrap', textOverflow: 'ellipsis', overflow: 'hidden' }}>{m.body}</div>}
