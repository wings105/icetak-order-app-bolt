...
export default function OrderItemStructuralEditor({ orderDbId, items: sourceItems, canEdit, structuralLocked, structuralLockReason, onSaved }: Props) {
  ...
  return <div className="struct-items">
    ...
    {drafts.map((draft, index) => {
      ...
      return <section className="struct-item-card" key={draft.clientId}>
        ...
        <div className="struct-grid">
          ...
          <label><span>Process</span><select disabled={!canEdit} value={draft.process} onChange={(e) => { update(draft.clientId, { process: e.target.value }); window.setTimeout(() => autoPrice({ ...draft, process: e.target.value }), 0); }}>{cfg.process.map((p) => <option key={p}>{p}</option>)}</select></label>
          <label><span>Size</span><select disabled={!canEdit} value={draft.size} onChange={(e) => { const size = e.target.value; const nextStyles = adminProductStyles(draft.k, size); const style = nextStyles.includes(draft.style) ? draft.style : nextStyles[0]; const next = { ...draft, size, style }; update(draft.clientId, { size, style }); window.setTimeout(() => autoPrice(next), 0); }}>{cfg.sizes.map((s) => <option key={s}>{s}</option>)}</select></label>
          <label><span>Style</span><select disabled={!canEdit} value={draft.style} onChange={(e) => { const next = { ...draft, style: e.target.value }; update(draft.clientId, { style: e.target.value }); window.setTimeout(() => autoPrice(next), 0); }}>{styles.map((s) => <option key={s}>{s}</option>)}</select></label>
          ...
          <label><span>Qty</span><input type="number" min="1" disabled={!canEdit} value={draft.qty} onChange={(e) => update(draft.clientId, { qty: Math.max(1, Number(e.target.value || 1)) })} /></label>
          <label><span>Unit Price</span><div className="struct-price"><input type="number" min="0" step="0.01" disabled={!canEdit} value={draft.price} onChange={(e) => update(draft.clientId, { price: Math.max(0, Number(e.target.value || 0)) })} /><button type="button" disabled={!canEdit} onClick={() => autoPrice(draft)}>Auto</button></div></label>
          <label className="wide"><span>Custom Text / Wording</span><input disabled={!canEdit} value={draft.customText} onChange={(e) => update(draft.clientId, { customText: e.target.value })} /></label>
          {draft.reviewRequired && <label className="wide"><span>Design Preview URL</span><input disabled={!canEdit} value={draft.previewUrl} onChange={(e) => update(draft.clientId, { previewUrl: e.target.value })} /></label>}
        </div>
    ...
    <div className="struct-footer"><div><span>Items subtotal</span><b>{money(total)}</b><small>Delivery fee is added by the order total backend.</small></div>{canEdit && <button className="btn btn-primary" disabled={saving || !drafts.length} onClick={() => void save()}>{saving ? 'Saving…' : 'Save Order Items'}</button>}</div>
  </div>;
}
