      ...
      <aside className={`sidebar ${mobileOpen ? 'mobile-open' : ''}`}>
            ...
            return (
              <div key={item.key}>
                ...
                >
                  ...
                  {item.children && (
                    <svg className={`sidebar-chevron ${isOpen ? 'open' : ''}`} width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="m9 18 6-6-6-6"/></svg>
                  )}
                </button>
                ...
                {item.children && isOpen && (
                  <div className="sidebar-subnav">
                    {item.children.map((child) => (
                      <button
                        key={child.key}
                        className={`sidebar-subitem ${active === child.key ? 'active' : ''}`}
        ...
        <div className="sidebar-footer">
          <button className="sidebar-item" style={{ opacity: 0.7 }}>
            <span className="sidebar-item-icon"><IconLogout size={16} /></span>
