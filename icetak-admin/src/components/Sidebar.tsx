...
export default function Sidebar({ active, onNavigate, mobileOpen, onCloseMobile, onLogout, canViewFinance = false, canViewCustomers = false, canViewPickup = false }: Props) {
  ...
  const [followupAttention, setFollowupAttention] = useState(0);

  useEffect(() => {
    let mounted = true;
    const load = async () => {
      const [clickup, shipping, followups] = await Promise.all([
        supabase.rpc('icetak_admin_clickup_queue_summary'),
        supabase.rpc('icetak_admin_shipping_attention_summary'),
        canViewFinance ? supabase.functions.invoke('finance-admin',{body:{action:'draft_followups',filter:'',limit:1}}) : Promise.resolve({data:null,error:null}),
      ]);
      if (!mounted) return;
      ...
      if (!followups.error) { const payload=followups.data as {success?:boolean;data?:FollowupSummary}|null; setFollowupAttention(Number(payload?.data?.counts?.due||0)+Number(payload?.data?.counts?.failed||0)); }
    };
    void load();
