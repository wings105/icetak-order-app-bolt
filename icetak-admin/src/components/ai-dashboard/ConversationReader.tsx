...
export default function ConversationReader({messages,boundary,onQuote,canQuote}:Props){
 const [query,setQuery]=useState(''),[filter,setFilter]=useState('all'),[copyStatus,setCopyStatus]=useState('');
 ...
 const filtered=ordered.filter(m=>(filter==='all'||filter==='media'&&!!m.media_url||m.direction===filter)&&(!term||`${m.text_content||''} ${m.caption||''}`.toLocaleLowerCase().includes(term.toLocaleLowerCase())));
 ...
 return <section className="ai-reader" aria-label="Conversation pelanggan">
  ...
  {boundary&&<div className="ai-session-banner">Sempadan session: {day(boundary)} · {time(boundary)}. Chat lama untuk rujukan sahaja.</div>}
